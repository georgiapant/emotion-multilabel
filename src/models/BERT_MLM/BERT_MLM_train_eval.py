...
from src.models.BERT_MLM.BERT_MLM_model import BertClassifier_MLM
...
class BertMLM:
    ...
    def initialize_model(self, train_dataloader, epochs, num_labels, BERT_MODEL):
        ...
        classifier = BertClassifier_MLM(num_labels, BERT_MODEL, freeze_bert=False)
    ...
    def mask_tokens(self, inputs, special_tokens_mask=None, mlm_probability=0.15):
        ...
        # 80% of the time, I replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).bool() & masked_indices
        inputs[indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)

        # 10% of the time, I replace masked input tokens with random word
    ...
    def validation(self, model, val_dataloader):
        ...
        for batch in val_dataloader:
            # Load batch to GPU
            b_input_ids, b_token_types, b_attn_mask, b_labels = tuple(t.to(self.device) for t in batch)

            # Compute logits
            with torch.no_grad():
                logits = model(b_input_ids,b_token_types, b_attn_mask)
            ...
            if self.weighted_loss:
                loss_cls = self.get_weighted_loss(logits, b_labels, self.weights)
            else:
                loss_cls = self.loss_fn(logits, b_labels)

            mask_ids, mask_lb = self.mask_tokens(b_input_ids.cpu())
            loss_mlm = model.mlmForward(mask_ids.to(self.device), b_token_types, b_attn_mask, mask_lb.to(self.device))

            loss = loss_cls + self.mlm_weight * loss_mlm
        ...
        for epoch_i in range(self.EPOCHS):
            ...
            if evaluation == True:
                ...
                if early_stopping.early_stop:
                    ...
                    thresholds_fin = np.mean(threshold_list[0:epoch_i - self.patience + 1], axis=0)
                    break
                else:
    ...
    def bert_predict(self, X_test, y_test=None):
        ...
        if self.weighted_loss:
            model = torch.jit.load(
                self.project_root_path + '/models/model_scripted_BERT_MLM_weighted_loss.pt', map_location=torch.device(self.device))
        else:
            model = torch.jit.load(self.project_root_path + '/models/model_scripted_BERT_MLM.pt', map_location=torch.device(self.device))
        ...
        for batch in test_dataloader:
            # Load batch to GPU
            b_input_ids, b_token_type, b_attn_mask = tuple(t.to(self.device) for t in batch)[:3]

            # Compute logits
            with torch.no_grad():
                logits = model(b_input_ids, b_token_type, b_attn_mask)
    ...
    def main(self):
        ...
        for batch in train_dataloader:
            b_input_ids, b_token_type, b_attn_mask = tuple(t.to(self.device) for t in batch)[:3]
            model_scripted = torch.jit.trace(bert_classifier, (b_input_ids, b_token_type, b_attn_mask))

            if self.weighted_loss:
                torch.jit.save(model_scripted,
                               self.project_root_path + '/models/model_scripted_BERT_MLM_weighted_loss.pt')
                break
            else:
                torch.jit.save(model_scripted, self.project_root_path + '/models/model_scripted_BERT_MLM.pt')
                break
